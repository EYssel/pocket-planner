...
contextBridge.exposeInMainWorld('planner', {
  ...
  offsetWeekKey: (key: string, delta: number) =>
    ipcRenderer.invoke('get-offset-week-key', { key, delta }),
  ...
  getPreviousWorkingDayKey: (dayKey: string) =>
    ipcRenderer.invoke('get-previous-working-day-key', dayKey),
  offsetDayKeyByWeeks: (dayKey: string, delta: number) =>
    ipcRenderer.invoke('offset-day-key-by-weeks', { dayKey, delta }),
  ...
  onSetMode: (cb: (mode: string) => void) =>
    ipcRenderer.on('set-mode', (_: any, mode: string) => cb(mode)),
  ...
  onUpdateAvailable: (cb: (version: string, isMac: boolean) => void) =>
    ipcRenderer.on('update-available', (_: any, version: string, isMac: boolean) =>
      cb(version, isMac),
    ),
  onUpdateNotAvailable: (cb: (version: string) => void) =>
    ipcRenderer.on('update-not-available', (_: any, version: string) => cb(version)),
  onCheckingForUpdates: (cb: () => void) => ipcRenderer.on('checking-for-updates', () => cb()),
  onUpdateProgress: (cb: (percent: number) => void) =>
    ipcRenderer.on('update-progress', (_: any, percent: number) => cb(percent)),
