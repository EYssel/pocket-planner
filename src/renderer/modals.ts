...
export async function handleCleanupAction(
  index: number,
  action: string,
  callbacks: { loadWeek: (key: string, skipStaleCheck: boolean) => Promise<void> },
) {
  state.setCleanupQueue(
    state.cleanupQueue.then(async () => {
      ...
      if (!sourceDay) return;
      const taskIndexInSource = sourceDay.plans.findIndex(
        (p: any) => p.text === task.text && !p.done,
      );
      ...
      if (action === 'carry') {
        ...
        if (todayWeekKey === state.currentWeekKey)
...
export async function renderRecycleBin() {
  ...
  if (bin.length === 0) {
    ui.recycleBinList.innerHTML =
      '<div style="padding: 20px; text-align: center; color: var(--muted);">The bin is empty.</div>';
    return;
  }
...
export async function handleBinAction(
  index: number,
  action: string,
  callbacks: { loadWeek: (key: string) => Promise<void> },
) {
  if (action === 'restore') {
    const bin = await window.planner.getRecycleBin();
