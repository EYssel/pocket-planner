...
export function setupEventListeners(callbacks: {
  ...
  ui.prevBtn?.addEventListener('click', async () =>
    callbacks.loadWeek(await window.planner.offsetWeekKey(state.currentWeekKey!, -1)),
  );
  ui.nextBtn?.addEventListener('click', async () =>
    callbacks.loadWeek(await window.planner.offsetWeekKey(state.currentWeekKey!, +1)),
  );
  ui.todayBtn?.addEventListener('click', async () =>
    callbacks.loadWeek(await window.planner.currentWeekKey()),
  );
  ...
  ui.openCleanup?.addEventListener('click', () => {
    modals.renderCleanupList();
    ui.cleanupModal.classList.add('show');
  });
  ui.closeBanner?.addEventListener('click', () => {
    ui.staleBanner.classList.remove('show');
  });
  ui.closeCleanup?.addEventListener('click', () => {
  ...
  ui.cleanupList?.addEventListener('click', (e: MouseEvent) => {
    ...
    if (btn)
      modals.handleCleanupAction(parseInt(btn.dataset.index!, 10), btn.dataset.action!, {
        loadWeek: callbacks.loadWeek,
      });
  });

  ui.openRecycleBin?.addEventListener('click', () => {
    modals.renderRecycleBin();
    ui.recycleBinOverlay.classList.add('show');
  });
  ui.closeRecycleBin?.addEventListener('click', () => {
  ...
  async function generateSummaryForDay(targetDayKey: string) {
    ...
    if (ui.summaryModalTitle) {
      ui.summaryModalTitle.textContent = isToday
        ? "Today's Summary"
        : `Summary for ${targetDayKey}`;
    }
    ...
    summaryPlainText =
      ...
      (prevDone.length > 0
        ? prevDone
            .map((p) => `- [DONE] ${p.text}${p.notes ? '\n' + formatNotesPlain(p.notes) : ''}`)
            .join('\n')
        : '- No completed tasks') +
  ...
  ui.recycleBinList?.addEventListener('click', (e: MouseEvent) => {
    const btn = (e.target as HTMLElement).closest('.action-btn') as HTMLButtonElement;
    if (btn)
      modals.handleBinAction(parseInt(btn.dataset.index!, 10), btn.dataset.action!, {
        loadWeek: callbacks.loadWeek,
      });
  });
  ...
  window.planner.onUpdateNotAvailable(() => {
    ...
    ui.updateStatus.innerHTML =
      '<strong>App Up to Date</strong> No updates available at this time.';
    ui.updateProgressContainer.style.display = 'none';
    ui.installUpdateBtn.style.display = 'none';
    ui.copyMacCmdBtn.style.display = 'none';
  ...
  ui.intervalSelect?.addEventListener('change', async (e: Event) => {
    await window.planner.setSetting(
      'notificationInterval' as any,
  ...
  ui.grid?.addEventListener(
    'focusout',
    (e: FocusEvent) => {
      if ((e.target as HTMLElement).classList.contains('task-edit')) {
        const dayKey = (e.target as HTMLElement)
          .closest('.task-item')
  ...
  ui.grid?.addEventListener('input', (e: Event) => {
    if ((e.target as HTMLElement).classList.contains('task-edit'))
      ui.autoResize(e.target as HTMLTextAreaElement);
  });
  ...
  ui.grid?.addEventListener('keydown', async (e: KeyboardEvent) => {
    ...
    const dayKey = item?.dataset.dayKey as string;
    const index = parseInt(item.dataset.index!, 10);
